...
The honest part first: this is a proof-of-concept. The default embedder and chat model are deterministic fakes, so the build and the whole test suite run with no API key and no Docker. The real providers (Azure OpenAI, pgvector) drop in through configuration without touching code. I built it to exercise the stack and get the architecture right end to end, and I wrote the trade-offs down in [LIMITATIONS.md](LIMITATIONS.md) instead of hiding them. The domain and the regulatory framing (CNJ Res. 615/2025, the LGPD) are specific to the Brazilian judiciary; [ADR 0012](Docs/adr/0012-regulatory-conformance-mapping.md) maps them without claiming conformance the PoC doesn't have.
...
You need the .NET 8 SDK (the .NET 10 SDK works too, since the project targets `net8.0`). Docker is optional.

```bash
# Runs with no key and no Docker: hash embeddings + extractive chat + in-memory store
...
No code changes, just configuration through user-secrets (full provisioning walkthrough in [`Docs/azure-setup.md`](Docs/azure-setup.md)):
...
- [`LIMITATIONS.md`](LIMITATIONS.md) — what this proof-of-concept does not do yet, in the first person, and how each gap closes.
...
## How I built this, and what I'd change

I built LexRAG as a focused spike, not a product. The constraint I set early was that it had to build, test, and run with no API key and no Docker, and that one rule shaped most of the architecture. Every model and every store sits behind a port with a deterministic fake, so the offline path and the real-provider path are the same code with a different DI registration. It also kept the hexagonal boundary honest, since the fakes live outside the core and the architecture test fails if anything leaks in.

The part I'm happiest with is the evaluation, because it reports its own limits. I show the LLM-judge variance instead of a single flattering number, and I left the score gate inert (`MinRelevanceScore = 0.0`) rather than ship a threshold I had no real data to calibrate.

What I'd change with more time: the hash embedder is lexical, so the semantic numbers only mean something after a real Azure run, and that run is what would populate them. I'd also calibrate that score gate on real data, since it's implemented but left at 0.0. The perturbation harness covers surface-form variants today; the next step there is semantic-paraphrase and adversarial-distractor variants, which need LLM-generated cases. None of those touch a port, which was the point.
