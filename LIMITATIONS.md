...
LexRAG is a proof-of-concept, not a production system. The items below cover what it does not do yet and how each gap would close. The same items, with file locations and escalation paths, are in the limitations table in [`Docs/architecture.md`](Docs/architecture.md).
...
Each item above has a corresponding [ADR](Docs/adr/) or row in the architecture limitations table, with an escalation path that does not change a port.
